...
{
    public readonly struct InstructionName
    {
        public InstructionType Type { get; }

        public InstructionName(InstructionType type)
        {
            Type = type;
        }

        public static InstructionName Parse(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
                throw new ArgumentNullException(nameof(name));
            if (!TryParse(name, out InstructionName result))
                throw new NotImplementedException($"The instruction name '{name}' is not implemented!");
            return result;
        }

        public static bool TryParse(string name, out InstructionName result)
        {
            InstructionType type = (name ?? string.Empty) switch
            {
            ...
            result = new InstructionName(type);
            return result.Type != InstructionType.None;
        }

        public static implicit operator InstructionName(InstructionType type) => new InstructionName(type);
        public static implicit operator InstructionName(string value) => Parse(value);
        public static explicit operator string(InstructionName name) => name.ToString();
